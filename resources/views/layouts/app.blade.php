...
<head>
    ...
    <meta name="description" content="@yield('meta_description', 'POS for Shops — Simple inventory and sales tracking for small retail businesses in Pakistan')">
...
">
    ...
    <div class="fixed top-4 right-4 z-[60] space-y-2 w-full max-w-sm px-4 md:px-0" dir="ltr">
        <template x-for="t in toast" :key="t.id">
            <div x-show="true" x-transition:enter="toast-enter" x-transition:leave="toast-exit"
                 :class="{
                ...
                <div class="flex items-start">
                    <div class="flex-shrink-0">
                        <svg x-show="t.type === 'success'" class="h-5 w-5 text-green-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
                        <svg x-show="t.type === 'error'" class="h-5 w-5 text-red-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>
                    </div>
                    <div class="ml-3">
                        <p class="text-sm font-medium" x-text="t.message"></p>
                    </div>
                    <button @click="toast = toast.filter(item => item.id !== t.id)" class="ml-auto -mx-1.5 -my-1.5 rounded-lg p-1.5 hover:bg-gray-100">
    ...
    <div class="min-h-screen flex flex-col md:flex-row">
        ...
        <!-- Mobile Header -->
        <div class="md:hidden bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-40 safe-top">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
                <img src="/images/logo.svg" alt="POS for Shops" class="h-8">
            </a>
            <div class="w-8"></div>
        </div>
                    ...
                    @if($plan)
                        <div class="bg-slate-800 rounded-lg p-3">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="text-xs text-blue-400">{{ $tenant->subscription('default')?->onTrial() ? 'Trial' : 'Active' }}</span>
                            </div>
                            <a href="{{ route('subscription-manager') }}" class="text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1">
        ...
        <main class="main-content flex-1 min-w-0">
            <header class="hidden md:flex items-center justify-between bg-white border-b border-gray-200 px-8 py-4 sticky top-0 z-30">
                <div>
        ...
        <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-40 safe-bottom pb-safe">
            <div class="flex justify-around items-center h-16">
                <a href="{{ route('dashboard') }}" class="flex flex-col items-center gap-1 p-2 {{ request()->routeIs('dashboard') ? 'text-blue-600' : 'text-gray-500' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                    <span class="text-[10px] font-medium">Dashboard</span>
                </a>
                <a href="{{ route('items.index') }}" class="flex flex-col items-center gap-1 p-2 {{ request()->routeIs('items.*') ? 'text-blue-600' : 'text-gray-500' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
                    <span class="text-[10px] font-medium">Items</span>
                </a>

                <div class="relative -top-4">
                    <button @click="$dispatch('open-quick-actions')" class="w-14 h-14 bg-blue-600 rounded-full shadow-lg shadow-blue-600/40 flex items-center justify-center text-white hover:bg-blue-700 active:scale-95 transition-all">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                    </button>
                </div>

                <a href="{{ route('stock-in') }}" class="flex flex-col items-center gap-1 p-2 {{ request()->routeIs('stock-in') ? 'text-blue-600' : 'text-gray-500' }}">
                ...
                <a href="{{ route('transactions') }}" class="flex flex-col items-center gap-1 p-2 {{ request()->routeIs('transactions') ? 'text-blue-600' : 'text-gray-500' }}">
