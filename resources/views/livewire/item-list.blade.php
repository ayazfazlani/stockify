    ...
    <div class="p-6 flex-1 min-h-screen overflow-y-auto" style="background: #F4F5F8;">
        ...
        <div class="flex justify-between items-center mb-6 max-sm:flex-wrap max-sm:gap-3">
            ...
            <div class="flex gap-2 max-sm:w-full max-sm:justify-between">
                @can('create items')
                @feature('bulk-import')
                <button wire:click="toggleImportModal" class="sf-btn sf-btn-green"
                    style="background: #12B76A; color: white;">
                    <i class='bx bx-import'></i>
                    <span>Import</span>
                </button>
                @endfeature
                <button wire:click="toggleModal" class="sf-btn sf-btn-blue" style="background: #4361EE; color: white;">
        ...
        <div class="flex flex-wrap items-center gap-4 mb-6 max-sm:justify-between">
            ...
            <div class="w-full flex justify-end md:flex-1 md:justify-normal">
                <button wire:click="$toggle('inStockOnly')"
                    ...
                    <div style="padding: 12px 14px;">
                        <div class="sf-item-list">
                            @forelse($items as $item)
                                <div wire:key="item-{{ $item->id }}" wire:click="selectItem({{ $item->id }})"
                            ...
                            <div class="space-y-5">
                                ...
                                @if($selectedItem->images && count($selectedItem->images) > 0)
                                    <div class="grid grid-cols-2 gap-3">
                                        @foreach($selectedItem->images as $img)
                                            <img src="{{ asset('storage/' . $img) }}" alt="{{ $selectedItem->name }}"
                                ...
                                @elseif($selectedItem->image)
                                    <img src="{{ asset('storage/' . $selectedItem->image) }}" alt="{{ $selectedItem->name }}"
                                        style="width: 100%; height: 160px; object-fit: contain; border-radius: 10px; border: 1px solid #E8EAF0; margin-bottom: 4px;">
                                @endif
                                    ...
                                    @can('view item cost')
                                        <div class="sf-field">
                                            ...
                                            <p style="font-family: 'JetBrains Mono', monospace;">
                                                ${{ number_format($selectedItem->cost, 2) }}</p>
                                        </div>
                                    @endcan
                                    <div class="sf-field">
                                        <label class="sf-label">Selling Price</label>
                                        <p style="font-family: 'JetBrains Mono', monospace; font-weight: 700;">
                                            ${{ number_format($selectedItem->price, 2) }}</p>
                                    </div>
                                    <div class="sf-field">
                                        <label class="sf-label">Quantity</label>
                                        <p class="{{ $selectedItem->quantity > 0 ? 'text-green-600' : 'text-red-600' }}"
                                ...
                                @can('edit items')
                                    <button wire:click="openEditModal" class="sf-btn sf-btn-blue"
        ...
        @if($isModalOpen)
            <div class="sf-overlay" wire:click.self="toggleModal">
                <div class="sf-modal">
                    ...
                    @if(!$canAddItems)
                        <div
                            style="margin: 0 18px; padding: 12px; background: #FFFAEB; border: 1px solid #FDE68A; border-radius: 10px; font-size: 13px; color: #92400E;">
                    ...
                    <div class="sf-modal-body {{ !$canAddItems ? 'opacity-50 pointer-events-none' : '' }}">
                        <div class="sf-field">
                            ...
                            @error('newItem.sku') <div class="sf-ferr"><i class='bx bx-error-circle'></i> {{ $message }}
                            </div> @enderror
                        </div>
                        ...
                        <div class="sf-row2">
                            <div class="sf-field">
                                <label class="sf-label">Cost Price</label>
                                <input type="number" step="0.01" wire:model="newItem.cost" placeholder="0.00"
                                    class="sf-finput">
                                @error('newItem.cost') <div class="sf-ferr">{{ $message }}</div> @enderror
                            </div>
                            <div class="sf-field">
                                <label class="sf-label">Selling Price</label>
                                <input type="number" step="0.01" wire:model="newItem.price" placeholder="0.00"
                                    class="sf-finput">
                        ...
                        <div class="sf-row2">
                            <div class="sf-field">
                                <label class="sf-label">Type</label>
                                <input type="text" wire:model="newItem.type" placeholder="e.g. Electronics"
                                    class="sf-finput">
                        ...
                        <div class="sf-field">
                            <label class="sf-label">Product Images <span class="opt">(optional, multiple
                                    allowed)</span></label>
                            @if($images)
