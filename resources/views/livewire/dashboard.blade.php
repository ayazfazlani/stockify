...
<style>
  ...
  *::after {
  ...
  /* scrollbar */
  .db-audit-list::-webkit-scrollbar {
    width: 4px;
  }

  .db-audit-list::-webkit-scrollbar-track {
    background: transparent;
  }

  .db-audit-list::-webkit-scrollbar-thumb {
...
<div class="db-wrap">
  ...
  <div class="db-main">
    ...
    <div class="db-stats">
      ...
      @if(isset($summary['inventoryEquity']))
        <div class="db-stat-card green">
          ...
          <div class="db-stat-value">
            {{ config('app.currency_symbol') }}{{ number_format($summary['inventoryEquity'], 0) }}</div>
        </div>
        ...
        <div class="db-stat-card amber">
          ...
          <div class="db-stat-value">
            {{ config('app.currency_symbol') }}{{ number_format($summary['potentialRevenue'], 0) }}</div>
        </div>
        ...
        <div class="db-stat-card purple">
          ...
          <div class="db-stat-value green">
            {{ config('app.currency_symbol') }}{{ number_format($summary['potentialProfit'], 0) }}</div>
        </div>
      @endif
              ...
              <tbody>
                @forelse($marginLeaders as $leader)
                  <tr>
                    ...
                    <td>
                      <span
                        class="db-profit">{{ config('app.currency_symbol') }}{{ number_format($leader['profit_pool'], 0) }}</span>
                    </td>
                  </tr>
                @empty
                  <tr>
                ...
                <div class="db-audit-item">
                  <div>
                    <div style="display:flex;align-items:center;gap:0">
                      <span
                        style="font-size:13px;font-weight:600;color:var(--text-1)">{{ $audit->item?->name ?? 'Item' }}</span>
                      <span
                        class="db-audit-action {{ strtolower($audit->action) === 'in' ? 'in' : (strtolower($audit->action) === 'out' ? 'out' : 'adj') }}">
                        {{ strtoupper($audit->action) }}
                      </span>
                    </div>
                    <div class="db-audit-who">{{ $audit->user?->name ?? 'System' }} ·
